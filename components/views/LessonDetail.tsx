...
export default function LessonDetail({ lesson }: LessonDetailProps) {
	...
	return (
		<div className="py-8 md:py-12 max-w-4xl mx-auto space-y-12">
			<Link
				href="/lessons"
				className="inline-flex items-center gap-2 text-[#7c1f31] dark:text-[#ff4d6d] font-bold hover:underline mb-4"
			>
							...
							<div className="bg-[#e7edeb] dark:bg-white/5 p-8 rounded-[2rem] border border-[#1a1a1a]/10 dark:border-white/10 flex flex-col sm:flex-row items-center gap-8">
								...
								<div className="flex-1 text-center sm:text-left">
									<h3 className="text-2xl font-bold text-[#1a1a1a] dark:text-white mb-2">
										Ești gata pentru verificare?
									</h3>
									<p className="text-[#1a1a1a]/70 dark:text-white/70 text-lg">
										Testează-ți cunoștințele acumulate în această lecție
										printr-un scurt quiz.
									</p>
								</div>
								<button
									onClick={startQuiz}
						...
						{!quizCompleted ? (
							<div className="space-y-10">
								<div className="flex items-center justify-between">
									<div className="space-y-1">
										<span className="text-sm font-black text-[#7c1f31] dark:text-[#ff4d6d] uppercase tracking-widest">
											Întrebarea {currentQuestionIndex + 1} din{" "}
											{currentQuiz.length}
										</span>
									...
									<div className="text-right">
										<span className="text-sm font-bold text-[#1a1a1a]/40 dark:text-white/40">
											Scor: {score}
										</span>
									</div>
								</div>
									...
									{currentQuiz[currentQuestionIndex].options.map(
										(option: string, index: number) => {
											...
											if (selectedOption === null) {
												buttonClass +=
													"border-[#1a1a1a]/10 dark:border-white/10 hover:border-[#7c1f31] dark:hover:border-[#ff4d6d] hover:bg-[#1a1a1a]/5 dark:hover:bg-white/5 bg-[#1a1a1a]/5 dark:bg-white/5";
											} else if (isSelected) {
												...
												buttonClass +=
													"border-[#1a1a1a]/5 dark:border-white/5 opacity-40";
											}
										...
										{selectedOption !==
											...
											<div className="bg-white/40 dark:bg-white/5 p-4 rounded-xl mb-4 border border-red-200 dark:border-red-800/30">
												...
												<p className="text-sm italic">
													{currentQuiz[currentQuestionIndex].remediation}
												</p>
											</div>
										)}
							...
							<div className="text-center space-y-8 py-10">
								...
								<div className="space-y-2">
									<h3 className="text-4xl font-black text-[#1a1a1a] dark:text-white">
										Felicitări!
									</h3>
									<p className="text-xl text-[#1a1a1a]/60 dark:text-white/60 font-medium">
										Ai finalizat quiz-ul pentru această lecție.
									</p>
								</div>
								<div className="bg-[#e7edeb] dark:bg-white/5 p-8 rounded-[2rem] border border-[#1a1a1a]/10 dark:border-white/10 shadow-sm max-w-sm mx-auto">
									<span className="text-sm font-black text-[#1a1a1a]/40 dark:text-white/40 uppercase tracking-widest">
										Scorul tău
									</span>
									<div className="text-6xl font-black text-[#7c1f31] dark:text-[#ff4d6d] my-2">
										{score}{" "}
										<span className="text-2xl text-[#1a1a1a]/20 dark:text-white/20">
											/ {currentQuiz.length}
										</span>
									</div>
									<p className="text-sm font-bold text-[#1a1a1a]/60 dark:text-white/60">
										{score === currentQuiz.length
											? "Ești un expert în digital literacy!"
											: "O performanță solidă. Continuă să înveți!"}
									</p>
								</div>
